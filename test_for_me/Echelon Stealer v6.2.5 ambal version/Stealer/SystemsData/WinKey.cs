...
namespace Echelon
{
    internal class WinKey
    {
        public static string GetDecodeKey(byte[] DigitalProductID)
        {
            ...
            for (var i = (int)Index.KeyIndexWin.KEY_START_INDEX; i <= (int)Index.KeyIndexWin.KEY_END_INDEX; i++) HexPid.Add(DigitalProductID[i]);
            var DecodedChars = new char[(int)Index.KeyIndexWin.DECODE_LENGTH];
            for (var i = (int)Index.KeyIndexWin.DECODE_LENGTH - 1; i >= 0; i--)
                if ((i + 1) % 0x6 != 0)
                    for (var j = (int)Index.KeyIndexWin.DECODE_STRING - 1; j >= 0; j--)
                    {
        ...
        public static string GetDecodeKeyWin8AndUp(byte[] DigitalProductId)
        {
            ...
            for (var i = 5; i < Result_Key.Length; i += 0x6) Result_Key = Result_Key.Insert(i, "-");
        ...
        public static string GetWindowsKey(string Path, string GetID)
        {
            ...
            var registryHive = RunChecker.IsAdmin ? RegistryHive.LocalMachine : RegistryHive.CurrentUser;
            var registryView = RunChecker.IsWin64 ? RegistryView.Registry64 : RegistryView.Registry32;
            try
            {
                using (var hklmHive = RegistryKey.OpenBaseKey(registryHive, registryView))
                using (var Key = hklmHive.OpenSubKey(Path, RunChecker.IsWin64))
                {
