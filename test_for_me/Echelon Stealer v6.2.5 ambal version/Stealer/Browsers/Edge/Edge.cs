    ...
    internal class Edge
    {
        ...
        public static void GetEdge(string Passwords)
        {
            ...
            try
            {
                ...
                if (result != 0) DesktopWriter.WriteLine(string.Format("[ERROR] Unable to enumerate vaults. Error (0x" + result + ")"));
                ...
                var vaultSchema = new Dictionary<Guid, string>
            {
                ...
                for (var i = 0; i < vaultCount; i++)
                {
                    ...
                    if (result != 0) DesktopWriter.WriteLine(string.Format($"Unable to open the following vault: {vaultType}. Error: 0x{result.ToString()}"));
                    ...
                    if (result != 0) DesktopWriter.WriteLine(string.Format($"[ERROR] Unable to enumerate vault items from the following vault: {vaultType}. Error 0x{result.ToString()}"));
                    ...
                    // For each vault item...
                        for (var j = 1; j <= vaultItemCount; j++)
                        {
                            ...
                            var VAULT_ITEM = OSMajor >= 6 && OSMinor >= 2 ? typeof(VaultCli.VAULT_ITEM_WIN8) : typeof(VaultCli.VAULT_ITEM_WIN7);
                            ...
                            if (result != 0) DesktopWriter.WriteLine(string.Format($"Error occured while retrieving vault item. Error: 0x{result.ToString()}"));
                            ...
                            if (pPackageSid != IntPtr.Zero && pPackageSid != null) packageSid = GetVaultElementValue(pPackageSid);
                            if (cred != null) // Indicates successful fetch
                            {
                                var resource = GetVaultElementValue(pResourceElement);
                                if (resource != null) DesktopWriter.WriteLine(string.Format("Url: {0}", resource));
                                var identity = GetVaultElementValue(pIdentityElement);
                                if (identity != null) DesktopWriter.WriteLine(string.Format("Username: {0}", identity));
                                if (packageSid != null) DesktopWriter.WriteLine(string.Format("PacakgeSid: {0}", packageSid));
