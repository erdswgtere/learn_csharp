	...
	public class CNT
	{
        public CNT(string baseName)
		{
			...
			if (File.Exists(baseName))
			{
				...
				var Value = Strings.Space((int)FileSystem.LOF(1));
				...
				if (decimal.Compare(new decimal(DataEncoding), decimal.Zero) == 0) DataEncoding = 1uL;
		...
		{
			if (rowIndex >= SqlRows.Length) return null;
            if (fieldIndex >= SqlRows[rowIndex].RowData.Length) return null;
        ...
        public bool ReadTable(string tableName)
		{
            ...
            var array = DataEntries[num].SqlStatement.Substring(DataEntries[num].SqlStatement.IndexOf("(") + 1).Split(',');
			var num3 = array.Length - 1;
			for (var j = 0; j <= num3; j++)
			{
				array[j] = array[j].TrimStart();
				var num4 = array[j].IndexOf(" ");
				if (num4 > 0) array[j] = array[j].Substring(0, num4);
                if (array[j].IndexOf("UNIQUE") == 0) break;
        ...
        private ulong ToUInt64(int startIndex, int Size)
		{
            ...
            var num = 0uL;
			for (var i = 0; i <= Size - 1; i++) num = (num << 8) | DataArray[startIndex + i];
            return num;
		}
        ...
        private long CalcVertical(int startIndex, int endIndex)
		{
			...
			for (var i = endIndex - 1; i >= startIndex; i += -1)
                if (i - 1 >= startIndex)
                {
		...
		{
			if (startIndex > DataArray.Length) return 0;
            var num = startIndex + 8;
			for (var i = startIndex; i <= num; i++)
			{
				if (i > DataArray.Length - 1) return 0;
                if ((DataArray[i] & 0x80) != 128) return i;
            }
			return startIndex + 8;
		}
        ...
        private void ReadDataEntries(ulong Offset)
		{
			...
			{
				var num = (ToUInt64((Offset.ForceTo<decimal>() + 3m).ForceTo<int>(), 2).ForceTo<decimal>() - decimal.One).ForceTo<ushort>();
				var num2 = 0;
				if (DataEntries != null)
				{
				...
				{
					var num4 = ToUInt64((Offset.ForceTo<decimal>() + 8m + (i * 2).ForceTo<decimal>()).ForceTo<int>(), 2);
					if (decimal.Compare(Offset.ForceTo<decimal>(), 100m) != 0) num4 += Offset;
                    var values = GetValues(num4.ForceTo<int>());
					CalcVertical(num4.ForceTo<int>(), values);
					var values2 = GetValues((num4.ForceTo<decimal>() + values.ForceTo<decimal>() - num4.ForceTo<decimal>() + decimal.One).ForceTo<int>());
					...
					values2 = values;
					var value = CalcVertical(num4.ForceTo<int>(), values);
					...
					do
					{
						...
						if (array[num5] > 9)
						{
							if (ItIsOdd(array[num5]))
                                array[num5] = (long)Math.Round((array[num5] - 13) / 2.0);
                            else
                                array[num5] = (long)Math.Round((array[num5] - 12) / 2.0);
                        }
						else
						{
					...
					var value2 = DataEncoding.ForceTo<decimal>();
					if (!decimal.One.Equals(value2))
					{
						if (!2m.Equals(value2))
						{
							if (3m.Equals(value2)) encoding = Encoding.BigEndianUnicode;
                        }
						else
						{
			...
			{
				if (DataArray[(uint)Offset] != 5) return;
				...
				{
					var num7 = (ushort)ToUInt64(Convert.ToInt32(decimal.Add(decimal.Add(new decimal(Offset), 12m), new decimal(j * 2))), 2);
        ...
        private bool ReadDataEntriesFromOffsets(ulong Offset)
		{
			if (DataArray[(uint)Offset] == 13)
			{
				var num = Convert.ToInt32(decimal.Subtract(new decimal(ToUInt64(Convert.ToInt32(decimal.Add(new decimal(Offset), 3m)), 2)), decimal.One));
				var num2 = 0;
				if (SqlRows != null)
				{
					num2 = SqlRows.Length;
				...
				{
					SZ[] array = null;
					var num4 = ToUInt64(Convert.ToInt32(decimal.Add(decimal.Add(new decimal(Offset), 8m), new decimal(i * 2))), 2);
					if (decimal.Compare(new decimal(Offset), 100m) != 0) num4 += Offset;
                    var values = GetValues((int)num4);
					CalcVertical((int)num4, values);
					var values2 = GetValues(Convert.ToInt32(decimal.Add(decimal.Add(new decimal(num4), decimal.Subtract(new decimal(values), new decimal(num4))), decimal.One)));
					...
					values2 = values;
					var num5 = CalcVertical((int)num4, values);
					var num6 = Convert.ToInt64(decimal.Add(decimal.Subtract(new decimal(num4), new decimal(values)), decimal.One));
					var num7 = 0;
					while (num6 < num5)
					{
						...
						if (array[num7].Type > 9)
						{
							if (ItIsOdd(array[num7].Type))
                                array[num7].Size = (long)Math.Round((array[num7].Type - 13) / 2.0);
                            else
                                array[num7].Size = (long)Math.Round((array[num7].Type - 12) / 2.0);
                        }
						else
						{
					...
					for (var j = 0; j <= num9; j++)
					{
						if (array[j].Type > 9)
						{
							if (!ItIsOdd(array[j].Type))
							{
                                ...
                                else if (decimal.Compare(new decimal(DataEncoding), 3m) == 0) SqlRows[num2 + i].RowData[j] = Encoding.BigEndianUnicode.GetString(DataArray, Convert.ToInt32(decimal.Add(decimal.Add(new decimal(num4), new decimal(num5)), new decimal(num8))), (int)array[j].Size);
                            }
							else
							{
								SqlRows[num2 + i].RowData[j] = Encoding.Default.GetString(DataArray, Convert.ToInt32(decimal.Add(decimal.Add(new decimal(num4), new decimal(num5)), new decimal(num8))), (int)array[j].Size);
			...
			else if (DataArray[(uint)Offset] == 5)
			{
				...
				{
					var num11 = (ushort)ToUInt64(Convert.ToInt32(decimal.Add(decimal.Add(new decimal(Offset), 12m), new decimal(k * 2))), 2);
