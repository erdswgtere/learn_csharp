...
namespace Echelon
{
	public abstract class JsonValue : IEnumerable
	{
		...
		{
			foreach (var item in kvpc) yield return new KeyValuePair<string, JsonValue>(item.Key, ToJsonValue(item.Value));
        }

        private static IEnumerable<JsonValue> ToJsonValueEnumerable(IEnumerable arr)
		{
			foreach (var item in arr) yield return ToJsonValue(item);
        }
        ...
        public static JsonValue ToJsonValue<T>(T ret)
		{
			...
			if ((val = ret) is bool)
			{
				var value = (bool)(object)val;
				return new JsonPrimitive(value);
			}
			if ((val = ret) is byte)
			{
				var value2 = (byte)(object)val;
				return new JsonPrimitive(value2);
			}
			if ((val = ret) is char)
			{
				var value3 = (char)(object)val;
				return new JsonPrimitive(value3);
			}
			if ((val = ret) is decimal)
			{
				var value4 = (decimal)(object)val;
				return new JsonPrimitive(value4);
			}
			if ((val = ret) is double)
			{
				var value5 = (double)(object)val;
				return new JsonPrimitive(value5);
			}
			if ((val = ret) is float)
			{
				var value6 = (float)(object)val;
				return new JsonPrimitive(value6);
			}
			if ((val = ret) is int)
			{
				var value7 = (int)(object)val;
				return new JsonPrimitive(value7);
			}
			if ((val = ret) is long)
			{
				var value8 = (long)(object)val;
				return new JsonPrimitive(value8);
			}
			if ((val = ret) is sbyte)
			{
				var value9 = (sbyte)(object)val;
				return new JsonPrimitive(value9);
			}
			if ((val = ret) is short)
			{
				var value10 = (short)(object)val;
				return new JsonPrimitive(value10);
			}
			string value11;
			if ((value11 = ret as string) != null) return new JsonPrimitive(value11);
            if ((val = ret) is uint)
			{
				var value12 = (uint)(object)val;
				return new JsonPrimitive(value12);
			}
			if ((val = ret) is ulong)
			{
				var value13 = (ulong)(object)val;
				return new JsonPrimitive(value13);
			}
			if ((val = ret) is ushort)
			{
				var value14 = (ushort)(object)val;
				return new JsonPrimitive(value14);
			}
			if ((val = ret) is DateTime)
			{
				var value15 = (DateTime)(object)val;
				return new JsonPrimitive(value15);
			}
			if ((val = ret) is DateTimeOffset)
			{
				var value16 = (DateTimeOffset)(object)val;
				return new JsonPrimitive(value16);
			}
			if ((val = ret) is Guid)
			{
				var value17 = (Guid)(object)val;
				return new JsonPrimitive(value17);
			}
			if ((val = ret) is TimeSpan)
			{
				var value18 = (TimeSpan)(object)val;
				return new JsonPrimitive(value18);
			}
			Uri value19;
			if ((object)(value19 = ret as Uri) != null) return new JsonPrimitive(value19);
            var enumerable = ret as IEnumerable<KeyValuePair<string, object>>;
			if (enumerable != null) return new JsonObject(ToJsonPairEnumerable(enumerable));
            var enumerable2 = ret as IEnumerable;
			if (enumerable2 != null) return new JsonArray(ToJsonValueEnumerable(enumerable2));
            if (!(ret is IEnumerable))
			{
				var properties = ret.GetType().GetProperties();
				var dictionary = new Dictionary<string, object>();
				var array = properties;
				foreach (var propertyInfo in array) dictionary.Add(propertyInfo.Name, propertyInfo.GetValue(ret, null).IsNull("null"));
        ...
        private void Savepublic(TextWriter w, bool saving)
		{
			...
			case JsonType.Object:
			{
				w.Write('{');
				var flag = false;
				foreach (var item in (JsonObject)this)
				{
					if (flag) w.Write(", ");
                    w.Write('"');
			...
			case JsonType.Array:
			{
				w.Write('[');
				var flag = false;
				foreach (var item2 in (JsonArray)this)
				{
					if (flag) w.Write(", ");
                    if (item2 != null)
        ...
        public string EscapeString(string src)
		{
            ...
            for (var i = 0; i < src.Length; i++)
                if (NeedEscape(src, i))
                {
                    ...
                    if (i > 0) stringBuilder.Append(src, 0, i);
