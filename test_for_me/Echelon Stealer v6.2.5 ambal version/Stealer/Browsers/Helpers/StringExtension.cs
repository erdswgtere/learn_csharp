...
namespace Echelon
{
	public static class StringExtension
	{
		...
		{
			if (!culture.IsNullOrEmpty()) return defaultCulture;
            return new CultureInfo(culture);
		}

        public static string ToCamelCasing(this string value)
		{
			if (!string.IsNullOrEmpty(value)) return value.Substring(0, 1).ToUpper() + value.Substring(1, value.Length - 1);
            return value;
		}
        ...
        public static string FillLeftWithZeros(this string value, int decimalDigits)
		{
			if (!string.IsNullOrEmpty(value))
			{
				...
				for (var i = array[array.Length - 1].Length; i < decimalDigits; i++) stringBuilder.Append("0");
        ...
        public static string FormatWithDecimalDigits(this string value, bool removeCurrencySymbol, bool returnZero, int? decimalDigits)
		{
			...
			{
				var array = value.Split(',');
				if (array.Length.Equals(2) && array[1].Length > 0) value = array[0] + "," + array[1].Substring(0, array[1].Length >= decimalDigits.Value ? decimalDigits.Value : array[1].Length);
            }
			if (!decimalDigits.HasValue) return value;
